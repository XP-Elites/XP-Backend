...
def test_upload_link_success(client):
    with patch(
        "upload_file.GitService.get_github_size", new_callable=AsyncMock
    ) as mock_size:
        mock_size.return_value = 10 * 1024 * 1024  # 10MB (under 100MB limit)

        response = client.post(
            "/upload/file_link/git", json={"git_link": "https://github.com/user/repo"}
        )
...
def test_upload_file_success(client):
    response = client.post(
        "/upload/files", files=[("files", ("test.txt", b"test content", "text/plain"))]
    )
